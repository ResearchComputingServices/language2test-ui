...
function User({ match }) {
    ...
    useEffect(() => {
        ...
        if (!_.isNil(rolesToCheck) && isAdminUser()) {
            const hasInstructorRole = rolesToCheck.some(role => _.eq(role.name, 'Instructor'));
            if (!hasInstructorRole) {
                setError('customError', 'enoent', 'Cannot remove the "Instructor" role from the "admin" user');
            } else {
                clearError();
            }
        }
